...
#include "arm_math.h"
#define MASK(X)   (1UL << (X))
#define BLUE_LED  19  // PTB21 and PTB22 for RED LED
...
//    PTB->PDDR |= MASK(BLUE_LED);
  ...
  while (1){
		...
		//if(result==1) PTB->PCOR = MASK(BLUE_LED);
		
		//else PTB->PCOR = MASK(BLUE_LED);
...
#define MASK(X)   (1UL << (X))
#define BLUE_LED  18  // PTB21 and PTB22 for RED LED
...
#define MASK(X)   (1UL << (X))
#define BLUE_LED  19  // PTB18 and PTB19 for RED LED
...
//parameters for operation between input vector and weigth1 matrix
	...
	while(1){
        ...
        *layer2BiasesQ,&output2_dims,*pOut2);
		...
		arm_fully_connected_s8(&ctx,&fc_params2,&quant_params,&input3_dims,*pOut2,&weight3_dims,*outputWeightsQ,&bias3_dims,           
